...
// System.out.println("after month "+month +"after year "+year);
 String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
    ...
    String region="",empsrlNo="",dataFlag="",frmName="",message="", accessCode="",userType="";
...
<html>
	<head>
	    ...
	    <script type="text/javascript">  
	     var accessCode='<%=accessCode%>',userType='<%=userType%>';
	function testSS() {
		var    url='', empsrlNo='' ;
    ...
    //alert(formType);
		...
		url="<%=basePath%>reportservlet?method=searchFor12MnthStatemntCtrn&accessCode="+accessCode+"&empsrlNo="+empserialNO+"&frmName="+'<%=frmName%>';
	...
	function editRecord(empserialNO,employeeName) {
		...
		url="<%=basePath%>reportservlet?method=get12MnthStatmntForCrtn&accessCode="+accessCode+"&frm_pensionno="+empserialNO+"&pageType="+pageType+"&frmName="+'<%=frmName%>';
  ...
  //alert("===empsrlNo=="+empsrlNo+"==message=="+message);
   ...
   function resetVals(){
	...
	function  statemntwagesreport(empSerialNo,pfidin78ps,reportFlag){ 
 	...
 	if(pfidin78ps==""){
 	if(reportFlag=="Log"){
 	alert("Please Edit ,Do Changes  and then Generate Log");
 	}else{
 	alert("Please Edit ,Do Changes  and then Generate Report");
 	}
	...
	var  sss = confirm("Do You want Report in Html/Excel Sheet Format. Click Ok For Html and Cancel For Excel Sheet");
 	...
 	url ="<%=basePath%>reportservlet?method=get12MnthStatmntForCrtn&frm_pensionno="+empSerialNo+"&frmName="+'<%=frmName%>'+"&frm_reportType="+reportType+"&pageType="+pageType+"&reportFlag="+reportFlag;
	 // alert(url);
   		...
   		<%boolean flag = false;%>
						...
						<table width="70%" border="0" align="center" cellpadding="1" cellspacing="0" class="tbborder">
							...
							<td height="5%" colspan="5" align="center" class="ScreenMasterHeading">
						 	12/60 Months Statement Corrections
							</td>
			 ...
			 <table width="70%" border="0" align="center" cellpadding="0" cellspacing="0" class="tbborder"> 
				   	  ...
				   	  <tr>							         
								...
								<td align="center" width="95%">
									...
									<display:column property="seperationReason" title="Seperation Reason"/>	
									<display:column property="dateofSeperationDate"  title="Seperation Date" />
									<display:column property="region"  title="Region" />	
									<display:column property="station"  title="Station" />									
									<display:column title="Log" media="html">
									 <a href="#" onclick="statemntwagesreport('<%=((EmpMasterBean)pageContext.getAttribute("advanceList")).getPfid()%>','<%=((EmpMasterBean)pageContext.getAttribute("advanceList")).getChkPfidIn78PS()%>','Log')"><img src="./PensionView/images/viewDetails.gif" border="0"  /></a>
								    </display:column>	
