...
<%
	...
	if(request.getAttribute("grandTotDiffShowFlag")!=null){
  	...
  	if(request.getAttribute("finlaizedFlag")!=null){
   	...
   	System.out.println("finlaizedFlag=="+finlaizedFlag);
  }
			...
			<%
						...
						for (int j = 0; j < pensionList.size(); j++) {
							...
							if (bean != null) {
								...
								count++;
  								countj=count;
								System.out.println("Emoluments==="+bean.getEmoluments());
							...
							System.out.println("advanceAmt==="+advanceAmt);
							pfwSubAmt =	Math.round(Double.parseDouble(bean.getEmployeeLoan()));
							System.out.println("pfwSubAmt==="+pfwSubAmt);
...
<option value="600" <%if(noofMonths.equals("600")){ out.println("selected");}%>>
			...
			<% 
				grandEmoluments = grandEmoluments
									...
									grandPensionInterest = grandPensionInterest
											+ pensionInterest;
									System.out.println("grandPensionInterest===="+grandPensionInterest);
			...
			<%
				...
				adjCrtnService.saveprvadjcrtntotalsforPc(empSerialNo, adjOBYear,form2Status,
							 ...
							 grandAAIContri,  grandAAIContriInterest, adjAAiContriInterest, adjPensionContriInterest,FSEmpSubIntrst,FSAAiContriIntrst,0);
  ...
  <input type="hidden" name="status" id="status">
	...
	<tr>
	 <td>
				...
				System.out.println("==PensionIntrst===prasant===="+PensionIntrst);
				if(grandTotDiffShowFlag.equals("true")){
					  result =  adjCrtnDAO.savepcadjcrtnCurrenttotalsforPc(empSerialNo,adjOBYear,transid,form2Status,grandEmoluments,grandCPF,grandCPFInterest,
					   										...
					   										emolumentsTot_diff,cpfTot_diff ,PenscontriTot_diff,PfTot_diff,empSubTot_diff,0,AAiContriTot_diff,0,0,Math.round(PensionIntrst),Math.round(empSubIntrst),Math.round(aaiContriIntrst),0,0);
					    }
					...
					<tr>
						<td class="HighlightData" align="center">AAIEPF Form-4</td>
						<td class="HighlightData"    align="right">&nbsp;</td>
