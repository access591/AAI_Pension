	  ...
	  var recoverieTable='<%=recoverieTable%>';
		 ...
		 if(buttonName=="S"){ 
			 ...
			 if(finaldate >= date1 && finaldate <= date2 &&recoverieTable!="true"){
	   	   		alert("FinalSettlement date can't be edited between 01-Apr-1995 and 31-Mar-2010. i.e PC Report 1995-2008 and PFCARD 2008-09 and 2009-10 already freezed");
		       return false;
				} 
...
<%if(!recoverieTable.equals("true")){ %>
					<td class=""> <input type="text"  size="10"  name="interestCalc"  value='<%=contr.getFinalSettlementDate()%>'>
<%}else{ %>
     <td class=""> <input type="text"  size="10"  name="interestCalc"  value='<%=contr.getInterestCalUpto()%>'>
<%} %>
                    <a onclick="interestCal();"><img src="<%=basePath%>PensionView/images/calendar.gif" border="no" /></a>
                    <input type="button"  name="editinterestdate"  value="E" onclick="editInterestDate('<%=pensionNo%>','interestCal')" /></td>
		...
		<table border="1" style="border-color: gray;" cellpadding="2"
			...
			<tr>
				...
				<td class="Data" width="6%" align="right"><input type="text"
					...
					size="5" value='<%=Math.round(Double.parseDouble(bean.getDbPensionCtr()))%>'></input></td>
  ...
  <input type="hidden" name="status" >
		...
		<tr>
			...
			<td align="center"><input type="button" class="btn"
				value="Submit" class="btn" onclick="deleteTransaction();"></td>
		</tr>
		<tr>
