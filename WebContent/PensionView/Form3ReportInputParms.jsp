<%String path = request.getContextPath();
	...
	String forms[] = {"FORM-3-PFID","FORM-3-PS-PFID"};
	String[] year = {"1995","1996","1997","1998","1999","2000","2001","2002","2003","2004","2005","2006","2007","2008","2009","2010","2011","2012","2013","2014","2015","2016","2017","2018","2019","2020","2021","2022"};				
	String Form3PFID ="Report Format Like :- 	[Serial Number]	[Pension A/c No]		[CPF ACC No]		[Employee Number]		[Employee Name]	[Designation]		[Father's name or Husband name( in case of married women)]		[Basic wages, D.A.]		[Age at Entry(Years)]		[Date of birth]		[Date of Joining]		[Sex] 	[Date of entitlement	for membership]		[Seperation Reason]	[Date of Seperation]		[Pension Option]	[Remarks]		[Airport Name]	[Region]";				
	String Form3PSPFID ="Report Format Like :- 	[Sl. No.]	[Account No]		[Name of the employee (in block capitals)]	[Fathers name or Husband's name( in case of married women)]		[Basic wages, D.A. including cash valueof food concession and retaining allowance,if any ]		[Age at Entry(Years)]			[Date of birth]		[Sex]			[Date of entitlement	for membership]		[Remarks]	";				
...
function popupWindow(mylink, windowname)
 ...
 function LoadWindow(params){
	...
	function resetReportParams(){
		document.forms[0].action="<%=basePath%>reportservlet?method=loadform3params";
	...
	function validateForm(user) {
		var reportType="",url="",formTypesel="";
    	...
    	if(document.forms[0].select_region.selectedIndex>0){
			...
			/*if(document.forms[0].txt_empname.value!='' && document.forms[0].chk_empflag.checked==false){
			  		...
			  		document.forms[0].chk_empflag.focus();
			  		return false;
			  }
			 if(document.forms[0].txt_empname.value=='' && document.forms[0].chk_empflag.checked==true){
		...
		if(document.forms[0].formType.value==''){
			...
			if(document.forms[0].formType.selectedIndex>0){
			formTypesel=document.forms[0].formType.options[document.forms[0].formType.selectedIndex].text;
			}else{
			formTypesel=document.forms[0].formType.value;
		 ...
		 //  alert("params "+params);
			...
			}else if(formTypesel=='FORM-3-PS-PFID'){
			if(sortingOrder=='cpfaccno'){
				sortingOrder='cpfacno';
			}
			url = "<%=basePath%>reportservlet?method=form3pfid&frm_region="+regionID+"&frm_year="+yearID+"&frm_month="+monthID+"&frm_reportType="+reportType+"&frm_formType="+formtype+"&frm_airportcd="+airportcode+"&sortingOrder="+sortingOrder;
			}else if(formTypesel=='FORM-3-PFID'){
			if(sortingOrder=='cpfaccno'){
				sortingOrder='cpfacno';
			}
				url = "<%=basePath%>reportservlet?method=form3pfid&frm_region="+regionID+"&frm_year="+yearID+"&frm_month="+monthID+"&frm_reportType="+reportType+"&frm_formType="+formtype+"&frm_airportcd="+airportcode+"&sortingOrder="+sortingOrder;
				...
				%>
															...
															<!-- <tr>
																...
																<tr>
																	...
																	<td>
																		<select name='formType' Style='width:100px' align="left" onmouseleave=showHideTooltip() onmouseenter=showHideTooltip()>
																			<option value=''>
																				[Select One]
																			</option>
																			 
																			<option value="<%=Form3PFID%>">FORM-3-PFID	</option>
																			<option value="<%=Form3PSPFID%>">FORM-3-PS-PFID </option>
